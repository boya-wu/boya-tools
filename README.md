...
### 🎙️ AI 逐字稿音訊轉檔與最佳化
- **Whisper / LLM 最佳化**：一鍵將各種影音檔案轉為 16kHz 單聲道 MP3 / M4A，自動限制在 25MB 以內（Whisper API 門檻）。
- **超輕量語音壓縮**：64kbps 語音頻率精算最佳化，體積縮減 80-90%，大幅降低 API 上傳耗時。
- **彈性參數與 GPU 加速**：支援 MP3/M4A/WAV/OGG 格式、取樣率、聲道自訂，並整合 GPU 硬體加速。
